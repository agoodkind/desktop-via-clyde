...
func TestInstallDryRunUsesShallowGhCloneAndOriginMain(t *testing.T) {
	...
	assertContains(t, log, "codex-cli: Cargo build will resolve upstream Rusty V8 artifact overrides when needed")
	assertContains(t, log, "codex-cli: release mode preserves the upstream release profile exactly")
	assertContains(t, log, "cargo build --manifest-path "+filepath.Join(sourceDir, "codex-rs", "Cargo.toml"))
