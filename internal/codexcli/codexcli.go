...
const (
	// StandaloneInstallCapability is the operation capability for local Codex CLI installs.
	StandaloneInstallCapability = "standalone-cli.install"
	// StandaloneStatusCapability is the operation capability for local Codex CLI status.
...
func RegisterOperations() error {
	if !catalog.HasOperationCapability(StandaloneInstallCapability) {
		if err := catalog.RegisterOperationCapability(StandaloneInstallCapability); err != nil {
			return logCodexCLIRegistrationError("register standalone install capability", err)
		}
	}
	if err := operations.Register(StandaloneInstallCapability, InstallOperation); err != nil {
		return logCodexCLIRegistrationError("register standalone install operation", err)
	}
	if !catalog.HasOperationCapability(StandaloneStatusCapability) {
		if err := catalog.RegisterOperationCapability(StandaloneStatusCapability); err != nil {
			return logCodexCLIRegistrationError("register standalone status capability", err)
		}
	}
	if err := operations.Register(StandaloneStatusCapability, StatusOperation); err != nil {
		return logCodexCLIRegistrationError("register standalone status operation", err)
	}
	return nil
}
...
func InstallOperation(ctx context.Context, req operations.Request) error {
	if err := Install(ctx, InstallOptions{
		...
		return fmt.Errorf("standalone cli install operation: %w",
			operations.Error(ctx, "operations.standalone_install_failed", "install standalone cli", err))
	}
	return nil
}
...
func StatusOperation(ctx context.Context, req operations.Request) error {
	if err := Status(ctx, StatusOptions{
		...
		codexcliLog.ErrorContext(ctx, "codexcli.status_operation_failed", "err", err)
		return fmt.Errorf("standalone cli status operation: %w",
			operations.Error(ctx, "operations.standalone_status_failed", "print standalone cli status", err))
	}
	return nil
}

func logCodexCLIRegistrationError(message string, err error) error {
	codexcliLog.Error("codexcli.registration_failed", "message", message, "err", err)
