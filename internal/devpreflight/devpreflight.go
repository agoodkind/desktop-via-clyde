...
func Warn(ctx context.Context, progress clioutput.Progress, dryRun bool, t targets.Target) {
	...
	if err := CredentialsDiscoverer(); err != nil {
		warnCredentialsMissing(ctx, progress, t.ID, missingSummary)
		return
	}
	if policy.AutoGenerate && !dryRun {
		generateAssets(ctx, progress, t.ID, policy)
		return
	}
	notef(progress, fmt.Sprintf(
...
func generateAssets(ctx context.Context, progress clioutput.Progress, targetID string, policy targets.DevelopmentSigningPolicy) {
...
func warnCredentialsMissing(ctx context.Context, progress clioutput.Progress, targetID, missingSummary string) {
...
func notef(progress clioutput.Progress, message string) {
	if progress == nil {
		return
	}
